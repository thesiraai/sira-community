# Redis TLS Connection POC - Rails Runner Version
# Run with: bundle exec rails runner temp/redis-tls-poc/test_redis_tls_runner.rb
...
begin
  ...
  result = redis1.ping
  puts "[OK] Connection successful with basic ssl: true - Response: #{result}"
rescue => e
...
begin
  ...
  result = redis2.ping
  puts "[OK] Connection successful with ssl_params (OpenSSL objects) - Response: #{result}"
rescue => e
  puts "[FAIL] Connection failed: #{e.class}: #{e.message}"
  puts "  #{e.backtrace.first(3).join("\n  ")}"
end
...
begin
  ...
  result = redis3.ping
  puts "[OK] Connection successful with ssl_params + ca_file - Response: #{result}"
rescue => e
  puts "[FAIL] Connection failed: #{e.class}: #{e.message}"
  puts "  #{e.backtrace.first(3).join("\n  ")}"
end
...
begin
  ...
  result = redis4.ping
  puts "[OK] Connection successful with ssl_params + ca_cert (OpenSSL) - Response: #{result}"
rescue => e
  puts "[FAIL] Connection failed: #{e.class}: #{e.message}"
  puts "  #{e.backtrace.first(3).join("\n  ")}"
end
...
begin
  ...
  result = redis5.ping
  puts "[OK] Connection successful with rediss:// URL - Response: #{result}"
rescue => e
  puts "[FAIL] Connection failed: #{e.class}: #{e.message}"
  puts "  #{e.backtrace.first(3).join("\n  ")}"
end
